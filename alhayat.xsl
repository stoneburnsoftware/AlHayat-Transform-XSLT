...
<xsl:comment>googleoff: snippet</xsl:comment>
    ...
    <a title="{$share_title}" target="_blank"><div class="google" style="float:left;" height="35px" width="35px"/></a>
    <a title="{$share_title}" target="_blank"><div class="twitter" style="float:left;" height="35px" width="35px"/></a>
    <a title="{$share_title}" target="_blank"><div class="facebook" style="float:left;" height="35px" width="35px"/></a>
...
$('#social_media_holder .twitter').on('click', function(){
	window.open("http://www.twitter.com/share?url="+encodeURIComponent(document.location)+"&#38;t="+encodeURIComponent(document.title),'_blank', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=250,width=600');
});
...
function callLiveNews() {
	if (!livePosting) {
        var Mainroute = document.getElementById("Mainroute").value;
        livePosting = true;
        $.ajax({
            url: Mainroute + 'Home/getLiveAjax',
            type: 'POST',
            data: '',
            success: function (data) {
                livePosting = false;
                $('#BodyLiveNews').html(data);
                if ($('#IsUrgentTest').val() == "1") {
                    callUrgent();
                }
                else {
                    $('#urgent').hide();
